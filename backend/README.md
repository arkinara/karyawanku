...
| GET | `/businesses/:id/work-location` | Semua role dalam bisnis | Lokasi kerja geofence `{ work_latitude, work_longitude, work_radius_m, geofence_mode }`. Belum dikonfigurasi → `404 { error: 'not_configured' }` |
| PATCH | `/businesses/:id/work-location` | business.manage (owner) | Set lokasi kerja + radius + mode geofence (ticket #67). Body parsial `{ work_latitude?, work_longitude?, work_radius_m?, geofence_mode? }`; lat/lon wajib berpasangan (keduanya angka ATAU keduanya null — null menonaktifkan lokasi); radius divalidasi 20–5000 m (default). 422 pada pelanggaran, 403 untuk non-owner |
...
| POST | `/attendance/clock-in` | Karyawan / Owner | Clock-in: `{ employee_id?, catatan?, client_timestamp?, submission_method?, latitude?, longitude?, accuracy_m? }`, hitung status + late_minutes otomatis dari shift. Waktu otoritatif = jam server (ticket #59). Koordinat + akurasi opsional; dievaluasi geofence server-side (ticket #67) |
| POST | `/attendance/clock-out` | Karyawan / Owner | Clock-out: `{ employee_id?, client_timestamp?, submission_method?, latitude?, longitude?, accuracy_m? }`, catat waktu keluar (jam server) + turunkan `overtime_minutes` dari clock-out melebihi jam selesai shift (+15 mnt grace, default; ticket #54). Geofence clock-out dievaluasi seperti clock-in |
...
Catatan integritas absensi (ticket #59): `clock_in`/`clock_out` yang tersimpan adalah **jam server** (`Date.now()` saat request diproses) — bukan waktu yang dikirim klien. Waktu dari klien (`client_timestamp`) disimpan terpisah sebagai klaim di `client_claim_at` (clock-in) / `clock_out_client_claim_at` (clock-out) dan **tidak pernah** menjadi waktu otoritatif. Toleransi selisih klaim vs jam server adalah konstanta bernama `TIMESTAMP_TOLERANCE_MS = 5 * 60 * 1000` (5 menit) di `src/routes/attendance.ts`. Klaim di masa depan melebihi toleransi ditolak 422; klaim `live` yang meleset ke masa lalu melebihi toleransi **diterima tapi ditandai** `time_drift_detected = true` untuk review, dan dicatat ke audit log dengan aksi `attendance.time_drift`. Antrian offline tetap berfungsi: flush `submission_method = 'offline_queue'` mempertahankan waktu aksi asli klien sebagai `clock_in`/`clock_out` (karena itu durasi offline yang sah) dan tidak ditandai drift. Guard identitas: employee yang mengirim `employee_id` selain miliknya ditolak 403 dan percobaannya dicatat ke audit log (`attendance.impersonation.blocked`); owner/manager (dengan `attendance.manage`) boleh mencatat untuk karyawan mana pun di bisnisnya. Entri/koreksi manual (`POST /api/attendance/manual`, `PATCH /api/attendance/:id`) tetap hanya untuk `attendance.manage` dan memakai waktu yang ditentukan owner/manual — tidak ada jalur `client_timestamp`.

Catatan geofence (ticket #67): owner mengonfigurasi lokasi kerja lewat `PATCH /api/businesses/:id/work-location` (`work_latitude` + `work_longitude` wajib berpasangan, `work_radius_m` 20–5000 m default 100, `geofence_mode` `'flag_only'` | `'block_in_radius'`). Evaluasi jarak dilakukan **murni server-side** dengan haversine (`src/lib/geofence.ts`): koordinat dicatat persis seperti dilaporkan klien (tidak pernah diinferensikan dari IP), dan verdict disimpan di `attendance_records.geofence_status` (`on_site`/`off_site`/`poor_accuracy`/`unknown`) + `clock_in_distance_m`/`clock_out_distance_m`. Klien **tidak** bisa melaporkan statusnya sendiri. Akurasi GPS > radius → `poor_accuracy` (GPS jelek ≠ di lokasi). Bisnis tanpa lokasi yang dikonfigurasi berperilaku persis seperti sebelumnya: `geofence_status = 'unknown'`, tanpa flag, tanpa blok. Mode `block_in_radius` menolak clock-in/out dengan `422` (tanpa record dibuat) bila di luar radius (`outside_geofence`), akurasi > radius (`accuracy_too_poor`), atau koordinat tidak dikirim sama sekali (`coordinates_required_for_blocking_business` — menghilangkan koordinat tidak bisa melewati geofence). `flag_only` menerima semua status dan hanya menandai. Nama field koordinat yang diterima: `latitude`/`longitude`/`accuracy_m` (kontrak) dan `lat`/`lng` (payload mobile #68). `GET /attendance/employee/:employeeId` dan `GET /attendance/today` mengembalikan kolom geofence pada record.
