...
void main() {
  ...
  test(
    'password sign-in mints + saves the device credential (with X-Device-Id)',
    () async {
      ...
      await container
          ...
          .signIn('siti@usaha.com', 'rahasia123');
  ...
  test(
    'device-refresh with the right device_id + a fresh biometric_proof returns a new session',
    () async {
      ...
      expect(
        body['device_refresh_token'],
        testDeviceCredential.deviceRefreshToken,
      );
      expect(
  ...
  test(
    'wrong device_id → 401 clears the credential and falls back to password',
    () async {
      await seedDeviceCredential(backend);
      await backend.write(DeviceIdentity.deviceIdKey, 'device-1');
      final auth = FakeAuthenticator(
  ...
  test(
    'expired credential → prompt still runs but no network request happens',
    () async {
  ...
  test(
    'cancelled biometric prompt → no network, stays signed out, credential kept',
    () async {
      await seedDeviceCredential(backend);
      await backend.write(DeviceIdentity.deviceIdKey, 'device-1');
      var deviceCalls = 0;
  ...
  test(
    'offline device-refresh → NetworkException keeps the credential',
    () async {
      await seedDeviceCredential(backend);
      await backend.write(DeviceIdentity.deviceIdKey, 'device-1');
      final auth = FakeAuthenticator(
