...
void main() {
  test('mints a fresh id at first launch and persists it across re-init', () async {
  ...
  test('rotate() drops the identity; the next init mints a fresh one', () async {
    final backend = InMemoryBackend();
  ...
  test('rememberPushToken persists the opaque device-bound string, rotate clears it', () async {
    final backend = InMemoryBackend();
    final identity = await DeviceIdentity.ensureInitialized(backend: backend);
  ...
  test('unreadable storage degrades to a blank identity, not a crash', () async {
...
class _ThrowingBackend implements SecureStorageBackend {
  ...
  @override
  Future<void> write(String key, String value) async => throw Exception('unreachable');
