...
void main() {
  ...
  test('availableKind returns fingerprint when a fingerprint is enrolled', () async {
    final service = BiometricService(
  ...
  test('isEnrolledChanged records the snapshot on the first call and is false', () async {
    final service = BiometricService(
      authenticator: FakeAuthenticator(kinds: [BiometricKind.fingerprint]),
      backend: backend,
    );
  ...
  test('isEnrolledChanged is true when the stored snapshot differs from the current one', () async {
  ...
  test('promptUnlock delegates to the authenticator and returns its verdict', () async {
  ...
  test('promptEnrolDecision is exactly-once even when dismissed (headless → false)', () async {
    final service = BiometricService(
      authenticator: FakeAuthenticator(kinds: [BiometricKind.fingerprint]),
      backend: backend,
    );
  ...
  test('promptEnrolDecision never asks when no biometric is enrolled', () async {
    final service = BiometricService(
      authenticator: FakeAuthenticator(),
      backend: backend,
    );
